...
In the benchmark scenario, there are no asset-backed securities. Because of limited short-selling, the market clears when the long-term asset's highest expected return matches the return of the short-term bond. Let $\mathcal{T}^0:=\{[0,\infty)\}$ and write $q^0:=q_{\mathcal{T}^0}$ for the underlying-asset price function associated with this no-securitization tranching. Notation is simplified by assuming that the asset price is \textit{cum-dividend} and that the declared dividend is to be paid next period.
...
\begin{eqnarray*}
p_{\tau^s}(m) + p_{\tau^j}(m) & > &  \frac{q^1(m)}{R}  \\
p_{\tau^s}(m) + p_{\tau^j}(m) & > &  \frac{\frac{1}{3}\left(q^1(l)+q^1(m)+q^1(h)\right)}{R}
\end{eqnarray*}
...
q^1(x) = \frac{d(x)}{R} + p_{\tau^s}(x) + p_{\tau^j}(x)
\end{equation}
...
As an example of a tranching that is not issuer-optimal, consider $\mathcal{T}^0=\left\{[0,\infty)\right\}$ in subsection \ref{benchmark}: there is no first-order stochastic dominance between beliefs at state $m$, as illustrated by Figure \ref{fig:CDFs}. On the other hand, the tranching $\mathcal{T}'=\left\{[0,q^1(m)),[q^1(m),\infty)\right\}$ in subsection \ref{securitized} is issuer-optimal: for each tranche and each state there is always an agent whose conditional cumulative distribution of the asset price caps the other one along the realizations in the tranche.
