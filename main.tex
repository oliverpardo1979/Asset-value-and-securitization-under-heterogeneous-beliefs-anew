...
In the benchmark scenario, there are no asset-backed securities. Because of limited short-selling, the market clears when the long-term asset's highest expected return matches the return of the short-term bond. Let $\mathcal{T}^0:=\{[0,\infty)\}$ and write $q^0:=q_{\mathcal{T}^0}:X\to\mathbb{R}_+$ for the underlying-asset price function associated with this no-securitization tranching. Notation is simplified by assuming that the asset price is \textit{cum-dividend} and that the declared dividend is to be paid next period.
