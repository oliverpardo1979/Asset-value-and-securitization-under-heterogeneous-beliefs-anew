...
The motivating example in Section \ref{example} fits this notation by taking $X=\{l,m,h\}$, a constant interest rate $R$, and $\mathbf{F}=\{\mathcal{A},\mathcal{B}\}$. In the limiting case where the true transition satisfies $P(x,x)=1-\epsilon$ and $P(x,y)=\epsilon/2$ for $y\neq x$, theory $\mathcal{A}$ corresponds to the coarse partition $\{X\}$ and theory $\mathcal{B}$ corresponds to the singleton partition $\{\{l\},\{m\},\{h\}\}$.
