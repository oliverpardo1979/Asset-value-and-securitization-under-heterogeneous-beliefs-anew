...
Assume now that any owner of the long-term asset can issue two kinds of ABS, each of which has a maturity of one period. The claims of the ABS add up to the realization of the payout of the long-term asset. Each ABS belongs to one of two tranches: the \textit{senior} tranche and the \textit{junior} tranche. Let $\mathcal{T}'=\{\tau_1,\tau_2\}$ denote this two-tranche structure and let $q':{\left\{l,m,h\right\}}\to\mathbb{R}_+$ be its asset price function. The senior tranche is $\tau_1=[0,q'(m))$ and the junior tranche is $\tau_2=[q'(m),\infty)$. The ABS in the senior tranche guarantees a payoff $q'(m)$ for next period unless the state becomes $l$, in which case it pays $q'(l)$. The payoff from the junior tranche is $q'(h)-q'(m)$ if the realized state is $h$ and $0$ otherwise. In short, writing $\underline{\tau}_k:=\inf(\tau_k)$ and $\overline{\tau}_k:=\sup(\tau_k)$ as in Section \ref{model}, the payoff functions from the two tranches are

\begin{align}
\phi_{\tau_1}(t) & = (t-\underline{\tau}_1)^+-(t-\overline{\tau}_1)^+
 =  t - (t-q'(m))^+, \label{eq:tau1payoff} \\
\phi_{\tau_2}(t) & = (t-\underline{\tau}_2)^+-(t-\overline{\tau}_2)^+
 =  (t-q'(m))^+, \label{eq:tau2payoff}
\end{align}
where $\overline{\tau}_2=\infty$ and the term $(t-\overline{\tau}_2)^+$ is interpreted as zero.
\begin{figure}[h]
\centering
...
\begin{eqnarray*}
p_{\tau_1}(m) + p_{\tau_2}(m) & > &  \frac{q'(m)}{R}  \\
p_{\tau_1}(m) + p_{\tau_2}(m) & > &  \frac{\frac{1}{3}\left(q'(l)+q'(m)+q'(h)\right)}{R}
\end{eqnarray*}
...
q'(x) = \frac{d(x)}{R} + p_{\tau_1}(x) + p_{\tau_2}(x)
\end{equation}
...
In the example of subsection \ref{benchmark}, there was no securitization so the tranching was given by $\mathcal{T}^0= \left\{ [0,\infty) \right\}$. In subsection \ref{securitized}, there were only two tranches and the attachment point was calibrated so that the tranching was $\mathcal{T}'=\{\tau_1,\tau_2\}$, where $\tau_1=[0,q'(m))$ and $\tau_2=[q'(m),\infty)$. Hence, the payoff functions in equations (\ref{eq:tau1payoff}) and (\ref{eq:tau2payoff}) are special cases of the generic expression in (\ref{tpayoff}).
