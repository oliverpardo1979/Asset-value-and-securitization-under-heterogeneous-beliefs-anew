...
This appendix provides the example summarized in Section \ref{sec:ep}, in which securitization can increase a theory's excess return even though that excess return remains negative. Consider a state space $X =\left\{l,m,h\right\}$ where the probability of a state going back to itself is close to one and uniform otherwise. The collection of theories is $\mathbf{F}=\left\{\mathcal{A},\mathcal{B},\mathcal{C}\right\}$, where:
\begin{eqnarray*}
\mathcal{A} & = &
\left\{
\left\{l\right\},
