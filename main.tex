...
Assume now that any owner of the long-term asset can issue two kinds of ABS, each of which has a maturity of one period. The claims of the ABS add up to the realization of the payout of the long-term asset. Each ABS belongs to one of two tranches: the \textit{senior} tranche and the \textit{junior} tranche. Let $\mathcal{T}'=\{\tau^s,\tau^j\}$ denote this two-tranche structure and let $q':{\left\{l,m,h\right\}}\to\mathbb{R}_+$ be its asset price function. The senior tranche is $\tau^s=[0,q'(m))$ and the junior tranche is $\tau^j=[q'(m),\infty)$. The ABS in the senior tranche guarantees a payoff $q'(m)$ for next period unless the state becomes $l$, in which case it pays $q'(l)$. The payoff from the junior tranche is $q'(h)-q'(m)$ if the realized state is $h$ and $0$ otherwise. In short, writing $\underline{\tau^a}:=\inf(\tau^a)$ and $\overline{\tau^a}:=\sup(\tau^a)$ for $a\in\{s,j\}$, as in Section \ref{model}, the payoff functions from the two tranches are

\begin{align}
\phi_{\tau^s}(t) & = (t-\underline{\tau^s})^+-(t-\overline{\tau^s})^+
 =  t - (t-q'(m))^+, \label{eq:tauspayoff} \\
\phi_{\tau^j}(t) & = (t-\underline{\tau^j})^+-(t-\overline{\tau^j})^+
 =  (t-q'(m))^+, \label{eq:taujpayoff}
\end{align}
where $\overline{\tau^j}=\infty$ and the term $(t-\overline{\tau^j})^+$ is interpreted as zero.
\begin{figure}[h]
\centering
...
\begin{eqnarray*}
p_{\tau^s}(m) + p_{\tau^j}(m) & > &  \frac{q'(m)}{R}  \\
p_{\tau^s}(m) + p_{\tau^j}(m) & > &  \frac{\frac{1}{3}\left(q'(l)+q'(m)+q'(h)\right)}{R}
\end{eqnarray*}
...
q'(x) = \frac{d(x)}{R} + p_{\tau^s}(x) + p_{\tau^j}(x)
\end{equation}
...
In the example of subsection \ref{benchmark}, there was no securitization so the tranching was given by $\mathcal{T}^0= \left\{ [0,\infty) \right\}$. In subsection \ref{securitized}, there were only two tranches and the attachment point was calibrated so that the tranching was $\mathcal{T}'=\{\tau^s,\tau^j\}$, where $\tau^s=[0,q'(m))$ and $\tau^j=[q'(m),\infty)$. Hence, the payoff functions in equations (\ref{eq:tauspayoff}) and (\ref{eq:taujpayoff}) are special cases of the generic expression in (\ref{tpayoff}).
